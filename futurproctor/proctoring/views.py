...
def exam(request):
    ...
    try:
        with open("E:\\Nirmal\\AI-Based-online-exam-proctoring-System\\futurproctor\\proctoring\\dummy_data//ai.json") as file:
            data = json.load(file)
...
def submit_exam(request):
    if request.method == 'POST':
        ...
        try:
            with open('E:\\Nirmal\\AI-Based-online-exam-proctoring-System\\futurproctor\\proctoring\\dummy_data\\ai.json', 'r') as file:
                data = json.load(file)
        except FileNotFoundError:
            return HttpResponse("Error: Questions file not found!", status=404)
        except json.JSONDecodeError:
